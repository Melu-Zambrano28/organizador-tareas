...
export const authMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!token || token !== 'Bearer my-secret-token') {    // Si no hay token o si el token no es exactamente "Bearer my-secret-token", entonces…   // Devuelve un error con código 401 (que significa "No autorizado").  // Manda un mensaje en JSON diciendo que no está autorizado.   // return detiene la función para que NO siga al siguiente paso.

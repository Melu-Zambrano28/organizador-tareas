...
export const validateMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!id || typeof id !== 'string') {
        res.status(400).json({error: "El campo 'id' es requerido y debe ser string"})
        return
    }
    if (!task || typeof task !== 'string') {
        res.status(400).json({error: "El campo 'task' es requerido y debe ser string"})
        return
    }
     if (!Date || typeof Date !== 'string') {
        res.status(400).json({error: "El campo 'Date' es requerido y debe ser string"})
        return
    }
    next()
}